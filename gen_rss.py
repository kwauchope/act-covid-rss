...
def normalise(locations):
    ...
    for location in locations:
        ...
        location['Exposure Site'] = re.sub(r"{location['Suburb']}", '', location['Exposure Site'], flags=re.IGNORECASE).strip()
        # Remove possible multiple white space in midle of string for mainly suburb removal but do it for everything
        {location[k] : space_re.sub(' ', v) for k, v in location.items()}
